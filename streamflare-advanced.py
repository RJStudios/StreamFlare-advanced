...
def download_media(link, output_dir, file_format, custom_filename=None, retries=3):
...
def main():
    ...
    links = input("Enter media links (comma-separated for batch processing): ").split(',')
    ...
    for link in progress_bar(links, total_links, desc="Downloading"):
        try:
            output_file = download_media(link.strip(), output_dir, file_format, custom_filename)
            if output_file:
